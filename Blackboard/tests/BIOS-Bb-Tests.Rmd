...
If your answer choices include superscript or subscript, the choice will be "pushed down" or "pushed up" relative to the answer numbering and selection button. To make it easier for students to visually tell which choice to select, add a "box" around the answer choices. Do this by selecting the image of a table in the middle row towards the right. Choose 1 row and 1 column. `r popups(mylink = "input/images/make-box.png")` Once your box appears, navigate to the the same table image and select `Table Properties` $\rightarrow$ `Advanced`. Select `Solid` border style and black color. `r popups(mylink = "input/images/make-border.png")` Once finished, you can change the size of the box with your cursor.
...
If you want the answer choices to appear in random order, **you must select the box for this option here**. `r popups(mylink = "input/images/mc-options.png")`
...
To reduce opportunities for cheating, it's nice to have several similar questions that vary slightly. Instead of creating each from scratch, you can make one first. From there, press the grey caret next to the question in the `Pool` and choose `Copy`. `r popups(mylink = "input/images/copy-question.png")` Make sure to delete the top "copy of" that Blackboard automatically adds in your new question. From here, you can easily change minor details of the question and select `Save`. The new question will be added to your pool.

## Tagging Questions

As you continue in the semester building pools and tests, your question bank will grow quite large. It can often be a challenge to find the question you would like to add to your test. To make life easier, remember to tag each question with either a `Category` or `Topic` while you create them. These options are usually found at the bottom of the screen after the answer choices. `r popups(mylink = "input/images/categories.png")` I recommend tagging by chapter (e.g., `Category` is `Chapter 8`) or content topic (e.g., `Topic` is `Meiosis`). These will be saved for future questions.
...
To add images to your answer choices, select `Question Settings` from the top menu panel from within your `Pool`. Make sure to check the boxes to allow images for answer choices (and/or feedback). 
`r popups(mylink = "input/images/question-settings.png")`

_Note: This setting takes some time to update!_

# Building Tests

Once you have completed the question pool, the next step is to build the test itself. In `Course Tools` $\rightarrow$ `Tests, Surveys, and Pools`, navigate to `Tests`. Select `Build Test`. Here, you can add a `Description` of the test and `Instructions` on how to complete it. (You can also add these later if you aren't yet ready.) Select `Submit`.
...
`r popups(mylink = "input/images/add-pool.png")`
...
`r popups(mylink = "input/images/ques-display.png")`
You can also edit the number of points that each question is worth.
`r popups(mylink = "input/images/ques-point.png")`

## Setting Point Values and Re-ordering Questions

If you wish to have students see questions at random, do not worry about the order of the questions as you build your test. However, if the order matters, you can use the up-and-down arrows on the upper right-hand corner to change the question order as you prefer. 
`r popups(mylink = "input/images/ques-order.png")`
...
`r popups(mylink = "input/images/ques-point.png")`
You can set any point value you like, but Blackboard will throw an error for decimals. Click `OK`, and then select `Submit` a second time. Try and keep the decimals to tenths only, to avoid headaches. For `Question Sets`, each question must have the same point value.
...
`r popups(mylink = "input/images/ques-totals.png")`
...
First, you may fill out a `Content Link Description`. This is separate from the original `Description` and `Instructions` you may have filled out previously. This is simply a description of the _link_ to the test, and it's usually unnecessary. If you already wrote your `Description` and `Instructions`, you'll see an option to show these to students before they begin the exam. I generally select both of these options. (If you did not fill these out previously, more on how to fill them out later below.) 
...
To add a `Description` or `Instructions`, click the grey caret next to the name of your test (top of the screen) from _within_ the `Test Options` page. To get to the `Test Options` page, select the grey caret next to the test itself in Blackboard (e.g., from within your Exams folder), and select `Edit the Test Options`. 

## Test Availability Exceptions

It is good practice to first create a `Group` (do this within `Users & Groups` and make sure it is **not** visible to students!) that contains the LOA students with their extension times. For example, a group titled `LOAs: 2x`. Under `Test Availability Exceptions` select `Add User or Group`. In the `Search` drop-down menu, select `Group` and simply press enter. All of the course groups will appear. Select the appropriate LOA groups.
...
`r popups(mylink = "input/images/test-exceptions.png")`

Here is a common scenario: a student is in the middle of taking an exam and has an internet issue. They contact you that they are re-starting the router but the time is ticking on their attempt. Add the student as an _individual_ exception here following the above procedure. (Select `Add User or Group` $\rightarrow$ Select the student.) If the exam is 60 minutes in length and you wish to give them 15 extra minutes, put 75 minutes in their `Timer`. This will give them an additional 15 minutes. To the student, it will appear that the clock is still counting down, but the clock will refresh with 15 more minutes once 60 minutes expires. 
...
1. `Score per question` shows the earned point values for each question. `r popups(mylink = "input/images/feedback-settings/only-score-per-question.png")`
2. `Show incorrect questions` will allow the students to see which questions they answered incorrectly. `r popups(mylink = "input/images/feedback-settings/score-per-question-and-incorrect.png")`
3. `All answers` shows all of the possible answers students could have selected (i.e., all answer choices). `r popups(mylink = "input/images/feedback-settings/scoreper-incorrect-allanswers.png")`
4. `Correct` shows correct answers for each of the questions. `r popups(mylink = "input/images/feedback-settings/scoreper-incorrect-allanswers-correct.png")`
5. `Submitted` shows only the answer that the student selected or submitted. `r popups(mylink = "input/images/feedback-settings/everythingchecked.png")`
...
`r smallfont("Version 5: Compiled by L E Delaney for UIC Biological Sciences Department")`
